"""Native cognis-connect emit for trackblock — forward findings to any platform.

Maps trackblock's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    trackblock ... --format json | trackblock-emit --to stix
    trackblock-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('ioc_id', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('family', '')
        out['description'] = rec.get('detail', '')
